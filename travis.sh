...
if [ "$TRAVIS_OS_NAME" = "linux" ]; then
    ...
    update_install(){
        sudo apt-get --allow-unauthenticated update && sudo apt-get --allow-unauthenticated install ghc-$GHCVER cabal-install-2.4 happy-1.19.4 alex-3.1.3
    }
    retry update_install
    export PATH=/opt/ghc/$GHCVER/bin:/opt/cabal/2.4/bin:/opt/happy/1.19.4/bin:/opt/alex/3.1.3/bin:$PATH
    retry cabal update
else
    brew update
