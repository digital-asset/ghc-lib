...
cHeaders ghcFlavor =
  ...
  [ "compiler/HsVersions.h" | ghcFlavor <= Ghc925] ++
...
parsersAndLexers ghcFlavor =
  map ("compiler" </>) $
      ...
      [ x |  ghcFlavor > Ghc925, x <- [ "GHC/Parser/HaddockLex.x", "GHC/Parser.hs-boot" ] ]
...
applyPatchDerivedConstants ghcFlavor =
    renameFileRewriteSrcs
      ...
      "DerivedConstants.h"
      (["compiler", stage0Compiler] ++ [stage0Rts | ghcFlavor > Ghc925])
      [".hs", ".y", ".hsc"]
...
applyPatchHaddockHs ghcFlavor = do
  ...
  when (ghcFlavor `elem` [Ghc921, Ghc922, Ghc923, Ghc924, Ghc925]) (
  ...
  when (ghcFlavor `elem` [Ghc923, Ghc924, Ghc925]) (
...
performExtraFilesSubstitutions ghcFlavor files =
  foldl' sub (files ghcFlavor) $
      [ ("rts/include/ghcversion.h", Nothing) | ghcFlavor > Ghc925 ] ++
      [ (hadrianGeneratedRoot ghcFlavor </> "ghcversion.h", Nothing) | ghcFlavor <= Ghc925] ++
      [  ((stage0Rts </> "include") </> "DerivedConstants.h", Just $ (stage0Rts </> "include") </> "GhclibDerivedConstants.h") | ghcFlavor > Ghc925] ++
      [ (hadrianGeneratedRoot ghcFlavor </> "DerivedConstants.h", Just $ hadrianGeneratedRoot ghcFlavor </> "GhclibDerivedConstants.h") | ghcFlavor <= Ghc925 ] ++
      [("compiler" </> "HsVersions.h", Just $ "compiler" </> "GhclibHsVersions.h") | ghcFlavor <= Ghc925 ]
  where
