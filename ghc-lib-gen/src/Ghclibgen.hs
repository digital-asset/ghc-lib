...
baseBounds ghcFlavor =
  case ghcFlavor of
    ...
    Ghc961   -> "base >= 4.16 && < 4.19" -- [ghc-9.2.1, ghc-9.7.1)

    GhcMaster -> "base >= 4.16 && < 4.19" -- [ghc-9.2.1, ghc-9.7.1)
...
performExtraFilesSubstitutions ghcFlavor files =
  foldl' sub (files ghcFlavor) $
      [
        if ghcFlavor > Ghc925 then
          ("rts/include/ghcversion.h", Nothing) -- now generated by ./configure
        else
          (hadrianGeneratedRoot ghcFlavor </> "ghcversion.h", Nothing)
      ] ++
      [
        if ghcFlavor > Ghc925 then
          ((stage0Rts </> "include") </> "DerivedConstants.h", Just $ (stage0Rts </> "include") </> "GhclibDerivedConstants.h")
        else
