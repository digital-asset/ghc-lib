...
cHeaders ghcFlavor =
  ...
  [ "compiler/HsVersions.h" | ghcFlavor <= Ghc922] ++
...
generatedHaddockLexer ghcFlavor =
  if ghcFlavor > Ghc922 then
    [stage0Compiler </> "GHC/Parser/HaddockLex.hs"]
  else
    []
...
applyPatchDerivedConstants ghcFlavor =
    renameFileRewriteSrcs
      ...
      "DerivedConstants.h"
      (["compiler", stage0Compiler] ++ [stage0Rts | ghcFlavor > Ghc922])
      [".hs", ".y", ".hsc"]
...
applyPatchHaddockHs ghcFlavor = do
  ...
  when (ghcFlavor == Ghc921 || ghcFlavor == Ghc922) (
