...
ghcLibIncludeDirs :: Bool -> GhcFlavor -> [FilePath]
...
#if __GLASGOW_HASKELL__ == 912  || __GLASGOW_HASKELL__ == 910
...
calcParserModules ghcFlavor = do
  ...
  let includeDirs = map ("-I" ++) (ghcLibParserIncludeDirs True ghcFlavor)
...
calcLibModules ghcFlavor = do
  ...
  lib <- mapM readCabalFile (cabalFileLibraries ghcFlavor)
  cabalPackageDb <- cabalPackageDb =<< ghcNumericVersion
  let includeDirs = map ("-I" ++) (ghcLibIncludeDirs True ghcFlavor)
...
applyPatchGHCRuntimeHeap ghcFlavor =
  when (ghcSeries ghcFlavor > GHC_9_14) $ do
    ...
    writeFile runtimeHeapInspectHs
      . replace
        "#if MIN_VERSION_ghc_heap(9,15,0)"
        (unlines rs <> "#if MIN_VERSION_ghc_heap(9,15,0)")
      =<< readFile' runtimeHeapInspectHs
  where
    runtimeHeapInspectHs = "compiler/GHC/Runtime/Heap/Inspect.hs"
...
baseBounds = \case
  ...
  GhcMaster ->
    ...
    "base >= 4.20 && < 4.23" -- [ghc-9.10.1, ghc-9.16.1)
