...
cHeaders ghcFlavor =
  ...
  [ "compiler/HsVersions.h" | ghcFlavor <= Ghc924] ++
...
parsersAndLexers ghcFlavor =
  map ("compiler" </>) $
      ...
      [ x |  ghcFlavor > Ghc924, x <- [ "GHC/Parser/HaddockLex.x", "GHC/Parser.hs-boot" ] ]
...
applyPatchDerivedConstants ghcFlavor =
    renameFileRewriteSrcs
      ...
      "DerivedConstants.h"
      (["compiler", stage0Compiler] ++ [stage0Rts | ghcFlavor > Ghc924])
      [".hs", ".y", ".hsc"]
...
applyPatchHaddockHs ghcFlavor = do
  ...
  when (ghcFlavor `elem` [Ghc921, Ghc922, Ghc923, Ghc924]) (
  ...
  when (ghcFlavor `elem` [Ghc923, Ghc924]) (
